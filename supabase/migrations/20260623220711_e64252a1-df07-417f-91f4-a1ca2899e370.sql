CREATE OR REPLACE FUNCTION public.get_all_time_leaderboard(_limit integer DEFAULT 50)
 RETURNS TABLE(rank integer, user_id uuid, display_name text, is_guest boolean, daily_wins integer, online_wins integer, best_daily_streak integer, score integer, is_me boolean)
 LANGUAGE sql
...
AS $function$
  ...
  combined AS (
    SELECT
      ...
      (COALESCE(d.wins, 0) * 20 + COALESCE(s.best_streak, 0) * 10 + COALESCE(o.wins, 0) * 5) AS score
  ...
  ranked AS (
    SELECT
      ROW_NUMBER() OVER (ORDER BY score DESC, daily_wins DESC, best_daily_streak DESC, display_name ASC)::int AS rank,
      *
    FROM combined
  )
  SELECT

export type Lang = 'en' | 'ar';

const translations = {
  en: {
    title: 'VAULT BREAKER',
    subtitle: 'Crack the Code. Break the Vault.',
    soloMode: 'Solo Game',
    onlineMode: 'Online Mode',
    trainingMode: 'Training',
    settings: 'Settings',
    codeLength: 'Code Length',
    allowDuplicates: 'Allow Duplicates',
    aiDifficulty: 'AI Difficulty',
    easy: 'Easy',
    medium: 'Medium',
    hard: 'Hard',
    difficulty: 'Difficulty',
    tierEasy: 'Easy',
    tierNormal: 'Normal',
    tierHard: 'Hard',
    tierLegendary: 'Legendary',
    botMode: 'Bot Mode',
    activeBot: 'Active Bot',
    passiveBot: 'Passive Bot',
    startMission: 'Solo Game',
    guess: 'Submit Guess',
    giveUp: 'Give Up',
    match: 'Match',
    shift: 'Shift',
    glitch: 'Glitch',
    attempt: 'Attempt',
    history: 'Decryption Log',
    digitTracker: 'Digit Tracker',
    digitTrackerHint: 'tap to mark',
    youWin: 'VAULT BREACHED!',
    youLose: 'ACCESS DENIED',
    aiWins: 'AI Breached Your Vault!',
    secretWas: 'The secret code was',
    newGame: 'New Mission',
    backToMenu: 'Back to Menu',
    enterDigit: 'Enter digit',
    invalidInput: 'Invalid input',
    noDuplicates: 'Duplicates not allowed',
    timeLeft: 'Time Left',
    yourVault: 'Your Vault',
    aiVault: "AI's Vault",
    yourTurn: 'Your Turn',
    aiTurn: "AI's Turn",
    turnsLeft: 'turns left',
    elapsed: 'Elapsed',
    language: 'Language',
    english: 'English',
    arabic: 'العربية',
    digits: 'digits',
    on: 'ON',
    off: 'OFF',
    comingSoon: 'Coming Soon',
    round: 'Round',
    maxTries: 'Max Tries',
    unlimited: 'Unlimited',
    outOfTries: 'Out of tries!',
    setYourSecret: 'Set Your Vault Code',
    setSecretHint: 'Choose a secret code for the AI to crack',
    confirmSecret: 'Lock Vault',
    // Online
    onlineSubtitle: 'Hack the vaults of real Breakers worldwide',
    createRoom: 'Create Room',
    joinRoom: 'Join Room',
    quickMatch: 'Quick Match',
    roomCode: 'Room Code',
    invalidCode: 'Invalid code format',
    nickname: 'Nickname',
    nicknamePlaceholder: 'Your handle',
    orSignIn: 'Or sign in for an account →',
    signIn: 'Sign In',
    signUp: 'Sign Up',
    signOut: 'Sign Out',
    password: 'Password',
    createAccount: 'Create Account',
    noAccount: "Don't have an account? Sign up",
    haveAccount: 'Already have an account? Sign in',
    continueAsGuest: 'Or continue as guest →',
    signedIn: 'Welcome back, Breaker',
    accountCreated: 'Account created. Check your email to verify.',
    waitingForOpponent: 'Waiting for Opponent',
    copyLink: 'Copy Share Link',
    copied: 'Link copied',
    shareCodeHint: 'Share the code or link with a friend',
    cancel: 'Cancel',
    findMatch: 'Find Match',
    matchMode: 'Match Mode',
    turnBased: 'Turn-based',
    simultaneous: 'Simultaneous',
    searchingOpponent: 'Searching for an opponent...',
    searchingHint: 'You will be paired with the next compatible Breaker',
    vaultLocked: 'Vault Locked',
    waitingOpponentSecret: 'Waiting for opponent to lock their vault...',
    opponentTurn: "Opponent's Turn",
    simultaneousRound: 'Submit Your Guess',
    opponentDisconnected: 'Opponent disconnected',
    opponentForfeited: 'Opponent Forfeited — You Win!',
    opponentWins: 'Opponent Breached Your Vault',
    opponentSecret: "Opponent's secret was",
    opponentGuesses: "Opponent's Attempts",
    forfeit: 'Forfeit Match',
    forfeitConfirm: 'Forfeit this match? Your opponent will win.',
    mustSignIn: 'Sign in to access this room',
    roomNotFound: 'Room not found',
    // Daily puzzle
    dailyPuzzle: 'Daily Puzzle',
    dailySubtitle: 'One puzzle. One chance. Resets at midnight Berlin time.',
    streak: 'Streak',
    bestStreak: 'Best',
    played: 'Played',
    wins: 'Wins',
    nextPuzzleIn: 'Next puzzle in',
    shareResult: 'Share Result',
    shareCopied: 'Result copied to clipboard',
    shareFailed: 'Could not share result',
    signInForCloudStreak: 'Sign in to save your streak across devices.',
    closeness: 'Closeness',
    bestGuess: 'Best guess',
    avgCloseness: 'Avg closeness',
    // Winner / rematch
    winnerIs: 'Winner',
    youWonShort: 'You won!',
    youLostShort: 'You lost',
    drawTitle: "It's a Draw",
    rematch: 'Rematch',
    rematchSent: 'Rematch sent — waiting for opponent...',
    rematchInviteTitle: 'Rematch invite',
    rematchInviteBody: 'Your opponent wants a rematch with the same settings.',
    accept: 'Accept',
    decline: 'Decline',
    playAgain: 'Play Again',
    you: 'You',
    aiPlayer: 'AI',
    // Stats
    myStats: 'My Stats',
    statsSubtitle: 'Your performance across all modes',
    onlineStats: 'Online Matches',
    dailyStats: 'Daily Puzzle',
    losses: 'Losses',
    draws: 'Draws',
    winRate: 'Win Rate',
    recentMatches: 'Recent Matches',
    noMatchesYet: 'No matches played yet. Jump online and break some vaults!',
    winShort: 'WIN',
    lossShort: 'LOSS',
    drawShort: 'DRAW',
    forfeitTag: 'forfeit',
    opponentLabel: 'Opponent',
    statsRequireAccount: 'Sign in to view your stats.',
    guestNoStats: 'Stats are for accounts only',
    createAccountToTrack: 'Create a free account to track your wins, losses, and streaks across devices.',
    statsMenu: 'My Stats',
    // Leaderboard
    leaderboardToday: "Today's Leaderboard",
    top20: 'Top 20',
    noSolversYet: 'No one has cracked it yet today. Be the first!',
    triesShort: 'tries',
    youTag: 'you',
    guestTag: 'guest',
    orSignInWith: 'Or sign in with',
    signInWithGoogle: 'Sign in with Google',
    // Battle Royale
    battleRoyale: 'Battle Royale',
    battleRoyaleSubtitle: 'Up to 8 players race the same secret',
    minPlayers: 'Min Players',
    waitingForPlayers: 'Waiting for players',
    startGame: 'Start Game',
    needMorePlayers: 'Need {n} more',
    players: 'Players',
    cracked: 'Cracked',
    outOfTriesShort: 'Out',
    gaveUp: 'Gave up',
    inGame: 'In game',
    nobodyWon: 'Nobody cracked it',
    secretCodeWas: 'Secret code',
    allAttempts: 'All Attempts',
    giveUpConfirm: 'Give up this round?',
    // Relay Race
    relayRace: 'Relay Race',
    relayRaceSubtitle: 'Two teams race to crack each other’s code',
    team: 'Team',
    yourTeam: 'Your Team',
    joinTeam: 'Join Team',
    unassigned: 'unassigned',
    eachTeamNeeds: 'Each team needs',
    waitingForHost: 'Waiting for host to start...',
    codeSettingPhase: 'Code Setting Phase',
    youAreSetter: 'You are picked to set the code',
    teamSecretLocked: 'Team code locked',
    waitingForSetter: 'Waiting for',
    opposingTeamChoosing: 'Opposing team is choosing their code...',
    theirSecret: 'Their secret',
    teamWins: 'YOUR TEAM WINS!',
    teamLost: 'Your team lost',
    winningTeam: 'Winning team',
    minPerTeam: 'Min / Team',
  },
  ar: {
    title: 'كاسر الخزنة',
    subtitle: 'فك الشفرة. اكسر الخزنة.',
    soloMode: 'لعبة فردية',
    onlineMode: 'أونلاين',
    trainingMode: 'تدريب',
    settings: 'الإعدادات',
    codeLength: 'طول الرمز',
    allowDuplicates: 'السماح بالتكرار',
    aiDifficulty: 'صعوبة الذكاء الاصطناعي',
    easy: 'سهل',
    medium: 'متوسط',
    hard: 'صعب',
    difficulty: 'الصعوبة',
    tierEasy: 'سهل',
    tierNormal: 'عادي',
    tierHard: 'صعب',
    tierLegendary: 'أسطوري',
    botMode: 'وضع البوت',
    activeBot: 'بوت نشط',
    passiveBot: 'بوت سلبي',
    startMission: 'لعبة فردية',
    guess: 'إرسال التخمين',
    giveUp: 'استسلام',
    match: 'تطابق',
    shift: 'إزاحة',
    glitch: 'خطأ',
    attempt: 'محاولة',
    history: 'سجل فك التشفير',
    digitTracker: 'متعقّب الأرقام',
    digitTrackerHint: 'انقر للتمييز',
    youWin: 'تم اختراق الخزنة!',
    youLose: 'تم رفض الوصول',
    aiWins: 'الذكاء الاصطناعي اخترق خزنتك!',
    secretWas: 'الرمز السري كان',
    newGame: 'مهمة جديدة',
    backToMenu: 'العودة للقائمة',
    enterDigit: 'أدخل رقم',
    invalidInput: 'إدخال غير صالح',
    noDuplicates: 'التكرار غير مسموح',
    timeLeft: 'الوقت المتبقي',
    yourVault: 'خزنتك',
    aiVault: 'خزنة الذكاء الاصطناعي',
    yourTurn: 'دورك',
    aiTurn: 'دور الذكاء الاصطناعي',
    turnsLeft: 'محاولات متبقية',
    elapsed: 'الوقت المنقضي',
    language: 'اللغة',
    english: 'English',
    arabic: 'العربية',
    digits: 'أرقام',
    on: 'تشغيل',
    off: 'إيقاف',
    comingSoon: 'قريباً',
    round: 'الجولة',
    maxTries: 'عدد المحاولات',
    unlimited: 'غير محدود',
    outOfTries: 'نفدت المحاولات!',
    setYourSecret: 'عيّن رمز خزنتك',
    setSecretHint: 'اختر رمزاً سرياً ليحاول الذكاء الاصطناعي كسره',
    confirmSecret: 'أقفل الخزنة',
    // Online
    onlineSubtitle: 'اخترق خزائن لاعبين حقيقيين حول العالم',
    createRoom: 'إنشاء غرفة',
    joinRoom: 'الانضمام لغرفة',
    quickMatch: 'مباراة سريعة',
    roomCode: 'رمز الغرفة',
    invalidCode: 'صيغة الرمز غير صحيحة',
    nickname: 'اللقب',
    nicknamePlaceholder: 'لقبك',
    orSignIn: 'أو سجل الدخول بحساب →',
    signIn: 'تسجيل الدخول',
    signUp: 'إنشاء حساب',
    signOut: 'تسجيل الخروج',
    password: 'كلمة المرور',
    createAccount: 'أنشئ الحساب',
    noAccount: 'ليس لديك حساب؟ أنشئ واحداً',
    haveAccount: 'لديك حساب؟ سجل الدخول',
    continueAsGuest: 'أو تابع كضيف →',
    signedIn: 'مرحباً بعودتك أيها الكاسر',
    accountCreated: 'تم إنشاء الحساب. تفقد بريدك للتأكيد.',
    waitingForOpponent: 'بانتظار الخصم',
    copyLink: 'انسخ رابط المشاركة',
    copied: 'تم النسخ',
    shareCodeHint: 'شارك الرمز أو الرابط مع صديق',
    cancel: 'إلغاء',
    findMatch: 'ابحث عن مباراة',
    matchMode: 'نمط المباراة',
    turnBased: 'بالتناوب',
    simultaneous: 'متزامن',
    searchingOpponent: 'جاري البحث عن خصم...',
    searchingHint: 'ستتم مطابقتك مع أول كاسر متوافق',
    vaultLocked: 'تم قفل الخزنة',
    waitingOpponentSecret: 'بانتظار الخصم لقفل خزنته...',
    opponentTurn: 'دور الخصم',
    simultaneousRound: 'أرسل تخمينك',
    opponentDisconnected: 'الخصم منقطع',
    opponentForfeited: 'انسحب الخصم — لقد فزت!',
    opponentWins: 'اخترق الخصم خزنتك',
    opponentSecret: 'الرمز السري للخصم كان',
    opponentGuesses: 'محاولات الخصم',
    forfeit: 'انسحب من المباراة',
    forfeitConfirm: 'انسحب من هذه المباراة؟ سيفوز الخصم.',
    mustSignIn: 'سجل الدخول للوصول لهذه الغرفة',
    roomNotFound: 'الغرفة غير موجودة',
    // Daily puzzle
    dailyPuzzle: 'اللغز اليومي',
    dailySubtitle: 'لغز واحد. فرصة واحدة. يُعاد تعيينه عند منتصف الليل بتوقيت برلين.',
    streak: 'سلسلة',
    bestStreak: 'الأفضل',
    played: 'لُعب',
    wins: 'انتصارات',
    nextPuzzleIn: 'اللغز التالي خلال',
    shareResult: 'شارك النتيجة',
    shareCopied: 'تم نسخ النتيجة',
    shareFailed: 'تعذّر مشاركة النتيجة',
    signInForCloudStreak: 'سجّل الدخول لحفظ سلسلتك عبر الأجهزة.',
    closeness: 'نسبة القرب',
    bestGuess: 'أفضل محاولة',
    avgCloseness: 'متوسط القرب',
    // Winner / rematch
    winnerIs: 'الفائز',
    youWonShort: 'لقد فزت!',
    youLostShort: 'لقد خسرت',
    drawTitle: 'تعادل',
    rematch: 'مباراة إعادة',
    rematchSent: 'تم إرسال طلب الإعادة — بانتظار الخصم...',
    rematchInviteTitle: 'طلب مباراة إعادة',
    rematchInviteBody: 'يريد خصمك مباراة إعادة بنفس الإعدادات.',
    accept: 'قبول',
    decline: 'رفض',
    playAgain: 'العب مجدداً',
    you: 'أنت',
    aiPlayer: 'الذكاء الاصطناعي',
    // Stats
    myStats: 'إحصائياتي',
    statsSubtitle: 'أداؤك عبر جميع الأنماط',
    onlineStats: 'مباريات أونلاين',
    dailyStats: 'اللغز اليومي',
    losses: 'هزائم',
    draws: 'تعادل',
    winRate: 'نسبة الفوز',
    recentMatches: 'مباريات أخيرة',
    noMatchesYet: 'لا توجد مباريات بعد. ادخل أونلاين واكسر بعض الخزائن!',
    winShort: 'فوز',
    lossShort: 'هزيمة',
    drawShort: 'تعادل',
    forfeitTag: 'انسحاب',
    opponentLabel: 'الخصم',
    statsRequireAccount: 'سجل الدخول لعرض إحصائياتك.',
    guestNoStats: 'الإحصائيات للحسابات فقط',
    createAccountToTrack: 'أنشئ حساباً مجانياً لتتبّع انتصاراتك وهزائمك وسلاسلك عبر الأجهزة.',
    statsMenu: 'إحصائياتي',
    // Leaderboard
    leaderboardToday: 'لوحة الصدارة اليوم',
    top20: 'أفضل 20',
    noSolversYet: 'لم يحلّها أحد بعد اليوم. كن الأول!',
    triesShort: 'محاولات',
    youTag: 'أنت',
    guestTag: 'ضيف',
    orSignInWith: 'أو سجل الدخول عبر',
    signInWithGoogle: 'تسجيل الدخول بجوجل',
    // Battle Royale
    battleRoyale: 'باتل رويال',
    battleRoyaleSubtitle: 'حتى 8 لاعبين يتسابقون على نفس الشفرة',
    minPlayers: 'الحد الأدنى للاعبين',
    waitingForPlayers: 'بانتظار اللاعبين',
    startGame: 'ابدأ اللعبة',
    needMorePlayers: 'يحتاج {n} أكثر',
    players: 'اللاعبون',
    cracked: 'فك الشفرة',
    outOfTriesShort: 'خرج',
    gaveUp: 'استسلم',
    inGame: 'في اللعبة',
    nobodyWon: 'لم يفك أحد الشفرة',
    secretCodeWas: 'الرمز السري',
    allAttempts: 'كل المحاولات',
    giveUpConfirm: 'هل تستسلم في هذه الجولة؟',
    // Relay Race
    relayRace: 'سباق التتابع',
    relayRaceSubtitle: 'فريقان يتسابقان لكسر شيفرة بعضهما',
    team: 'فريق',
    yourTeam: 'فريقك',
    joinTeam: 'انضم للفريق',
    unassigned: 'غير مخصص',
    eachTeamNeeds: 'كل فريق يحتاج',
    waitingForHost: 'بانتظار المضيف للبدء...',
    codeSettingPhase: 'مرحلة تعيين الشيفرة',
    youAreSetter: 'تم اختيارك لتعيين الشيفرة',
    teamSecretLocked: 'تم قفل شيفرة الفريق',
    waitingForSetter: 'بانتظار',
    opposingTeamChoosing: 'الفريق الخصم يختار شيفرته...',
    theirSecret: 'شيفرتهم',
    teamWins: 'فاز فريقك!',
    teamLost: 'خسر فريقك',
    winningTeam: 'الفريق الفائز',
    minPerTeam: 'الحد الأدنى / فريق',
  },
} as const;

export type TranslationKey = keyof typeof translations.en;
export default translations;
